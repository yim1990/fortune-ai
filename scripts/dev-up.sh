...
if [ ! -f ".env" ]; then
    ...
    if [ -f ".env.example" ]; then
        cp .env.example .env
        log_success ".env 파일이 생성되었습니다. 필요한 값들을 수정해주세요."
    else
...
if curl -s -X GET http://localhost:3000/api/convert-proxy > /dev/null 2>&1; then
    log_success "API 프록시가 정상적으로 작동 중입니다."
else
    log_warning "API 프록시에 연결할 수 없습니다."
fi
...
echo "  • PHP API 서비스: http://localhost:8080"
echo "  • PHP API 헬스체크: http://localhost:8080/healthz"
echo ""
echo "🔧 유용한 명령어:"
